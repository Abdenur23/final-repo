...
class RealTimeUpdates {
    constructor() {
        ...
        this.processedFiles = new Set();
        this.productPrice = 34;
        this.currentUploadSession = null; // Track current upload session
        this.sessionProgress = new Map(); // Track progress per session
    }
    ...
    startNewUploadSession() {
        ...
        this.currentUploadSession = sessionId;
        this.sessionProgress.set(sessionId, {
            hasDisplayedProduct: false,
            processedFiles: new Set(),
            pendingImages: new Map()
        });
        
        console.log('🆕 Started new upload session:', sessionId);
        return sessionId;
    }
    ...
    handleDesignReady(designData) {
        ...
        if (!this.completedDesigns.has(designId)) {
            ...
            // Mark that we've displayed a product for current session
            if (this.currentUploadSession) {
                const session = this.sessionProgress.get(this.currentUploadSession);
                if (session) {
                    session.hasDisplayedProduct = true;
                }
            }
        }
    }
    ...
    handleImageUpdate(update) {
        const fileKey = update.fileName + '_' + update.stage;
        
        // Check if we're in an active upload session
        if (!this.currentUploadSession) {
            console.log('No active upload session, ignoring update:', fileKey);
            return;
        }
        ...
        // Only show progress if no products have been displayed in THIS session
        if (!session.hasDisplayedProduct) {
            if (session.processedFiles.has(fileKey)) {
                console.log('Skipping duplicate file update in session:', fileKey);
                return;
            }
            session.processedFiles.add(fileKey);
            this.updateImageStatus(update);
        }
    }
    ...
    // Clear progress for new upload (call this when starting new upload)
    clearProgressForNewUpload() {
        // Clear only progress items, keep completed products
        const container = document.getElementById('realtimeUpdates');
        const progressItems = container.querySelectorAll('.progress-item');
        progressItems.forEach(item => item.remove());
