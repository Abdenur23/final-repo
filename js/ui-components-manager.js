class UIComponentsManager {
    ...
    // Add this method to UIComponentsManager class
    createProgressTrainItem(itemKey, friendlyName, stage, imageUrl = null) {
        const id = `progress-train-${itemKey}-${stage}`;
        const html = `
            <div id="${id}" class="progress-train-item">
                <div class="progress-train-content">
                    <div class="stage-name">${friendlyName}</div>
                    ${imageUrl ? `<img src="${imageUrl}" alt="${friendlyName}" class="progress-train-thumbnail" onload="this.style.opacity='1'" style="opacity: 0; transition: opacity 0.3s ease;">` : ''}
                    <div class="timestamp">${new Date().toLocaleTimeString()}</div>
                </div>
            </div>
        `;
